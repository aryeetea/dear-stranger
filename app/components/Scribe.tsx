...
const ENVELOPES = [
  ...
  { id: 'celestial', label: 'Celestial',  desc: 'Gold compass and stars' },
  { id: 'lace',      label: 'Lace',       desc: 'Delicate scalloped trim' },
  { id: 'citrus',    label: 'Citrus',     desc: 'Sunny orange paper' },
  { id: 'blueprint-air', label: 'Blueprint Air', desc: 'Sky mail technical edge' },
]
...
function EnvelopeSVG({ id = 'classic', color = '#c8a050', width: w = 120, height: h = 80 }: { id?: string; color?: string; width?: number; height?: number }) {
  ...
  if (id === 'celestial') return <svg width={w} height={h} viewBox="0 0 120 80"><rect x="2" y="20" width="116" height="58" rx="2" fill="#121933" stroke="rgba(210,185,110,0.42)" strokeWidth="1"/><path d="M2 20 L60 56 L118 20 Z" fill="#182044" stroke="rgba(210,185,110,0.32)" strokeWidth="0.8"/><path d="M2 78 L60 46 L118 78" fill="none" stroke="rgba(210,185,110,0.22)"/><circle cx="60" cy="47" r="11" fill="none" stroke="rgba(210,185,110,0.34)" strokeWidth="1"/><path d="M60 33 L63 43 L73 43 L65 49 L68 59 L60 53 L52 59 L55 49 L47 43 L57 43 Z" fill="rgba(255,232,170,0.82)"/>{[[14,30],[26,26],[37,40],[82,29],[95,37],[104,27],[90,62],[20,61]].map(([cx,cy],i)=><circle key={i} cx={cx} cy={cy} r={i%3===0?1.3:0.8} fill="rgba(255,236,180,0.78)"/>)} </svg>
  if (id === 'lace') return <svg width={w} height={h} viewBox="0 0 120 80"><rect x="6" y="20" width="108" height="58" rx="2" fill="#fff8f2" stroke="rgba(180,150,130,0.34)" strokeWidth="1"/><path d="M6 20 L60 56 L114 20 Z" fill="#fff3ea" stroke="rgba(180,150,130,0.26)" strokeWidth="0.8"/><path d="M6 78 L60 46 L114 78" fill="none" stroke="rgba(180,150,130,0.18)"/>{[10,18,26,34,42,50,58,66,74,82,90,98,106].map((x,i)=><circle key={i} cx={x} cy="20" r="4" fill="none" stroke="rgba(205,180,165,0.52)" strokeWidth="1"/>)}{[10,18,26,34,42,50,58,66,74,82,90,98,106].map((x,i)=><circle key={i} cx={x} cy="78" r="4" fill="none" stroke="rgba(205,180,165,0.52)" strokeWidth="1"/>)} </svg>
  if (id === 'citrus') return <svg width={w} height={h} viewBox="0 0 120 80"><rect x="2" y="20" width="116" height="58" rx="2" fill="#fff0d6" stroke="rgba(200,130,40,0.38)" strokeWidth="1"/><path d="M2 20 L60 56 L118 20 Z" fill="#ffe5bf" stroke="rgba(200,130,40,0.28)" strokeWidth="0.8"/><path d="M2 78 L60 46 L118 78" fill="none" stroke="rgba(200,130,40,0.18)"/><circle cx="18" cy="66" r="6" fill="rgba(255,184,64,0.5)"/><circle cx="102" cy="31" r="6" fill="rgba(255,184,64,0.5)"/><path d="M18 66 L22 61" stroke="rgba(100,140,60,0.6)" strokeWidth="1"/><path d="M102 31 L106 26" stroke="rgba(100,140,60,0.6)" strokeWidth="1"/></svg>
  if (id === 'blueprint-air') return <svg width={w} height={h} viewBox="0 0 120 80"><rect x="2" y="2" width="116" height="76" rx="2" fill="#edf6ff" stroke="#3b78b8" strokeWidth="2.4"/><rect x="8" y="8" width="104" height="64" rx="1" fill="none" stroke="#d65a5a" strokeWidth="1" strokeDasharray="5 4"/><path d="M2 20 L60 54 L118 20 Z" fill="#e4eef9" stroke="rgba(60,110,170,0.36)" strokeWidth="0.8"/><path d="M2 78 L60 48 L118 78" fill="none" stroke="rgba(60,110,170,0.24)"/><line x1="2" y1="20" x2="60" y2="46" stroke="rgba(60,110,170,0.14)" strokeWidth="0.6"/><line x1="118" y1="20" x2="60" y2="46" stroke="rgba(60,110,170,0.14)" strokeWidth="0.6"/><path d="M84 60 L104 40 L110 46 L90 66 Z" fill="none" stroke="rgba(60,110,170,0.35)" strokeWidth="1.2"/><text x="26" y="66" fontSize="7" fontFamily="serif" fill="#3b78b8" letterSpacing="1.5">AIR POST</text></svg>
...
export default function Scribe({ recipientName, senderName, draftOwnerId, lettersSent = 0, replyContext, onClose, onSend }: {
        ...
        <div style={selectedHandwriting === 'typed' ? getHandwritingStyleStyles(selectedHandwriting) : {}}>
          {selectedHandwriting === 'handwritten' ? (
            <div>
              ...
              <HandwritingCanvas ref={canvasRef} inkColor={effectiveInk.main} lineWidth={1.5} height={520} />
  ...
  const renderPickerHeader = (eyebrow: string, title: string, subtitle: string, confirmation: string) => (
    <div
      style={{
        position: 'sticky',
        top: '12px',
        ...
        padding: '14px 16px',
        borderRadius: '14px',
        background: 'linear-gradient(180deg, rgba(13,15,34,0.96), rgba(8,10,24,0.92))',
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px', flexWrap: 'wrap' }}>
        <div style={{ minWidth: 0 }}>
          <p style={{ fontFamily: "'Cinzel', serif", fontSize: '8px', letterSpacing: '0.34em', color: 'rgba(230,199,110,0.78)', textTransform: 'uppercase', margin: '0 0 6px' }}>
            {eyebrow}
          </p>
          <p style={{ fontFamily: "'IM Fell English', serif", fontStyle: 'italic', fontSize: '15px', color: 'rgba(255,255,255,0.92)', margin: '0 0 4px', lineHeight: 1.35 }}>
            {title}
          </p>
          <p style={{ fontFamily: "'IM Fell English', serif", fontStyle: 'italic', fontSize: '12px', color: 'rgba(255,255,255,0.56)', margin: 0, lineHeight: 1.4 }}>
            {subtitle}
          </p>
        </div>
        ...
        <button
          ...
          onMouseEnter={e => {
            e.currentTarget.style.background = 'rgba(230,199,110,0.16)'
            e.currentTarget.style.borderColor = 'rgba(230,199,110,0.58)'
          }}
          onMouseLeave={e => {
      ...
      <AnimatePresence mode="wait">
        {view==='papers' && (
          <motion.div key="papers" initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }} style={{ width:'min(760px, 95vw)', zIndex:2 }}>
            {renderPickerHeader('Paper', `Writing on ${selectedPaper.label}`, 'Pick a page style, then jump straight back to your draft.', `Use ${selectedPaper.label}`)}
        ...
        {view==='stamps' && (
          <motion.div key="stamps" initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }} style={{ width:'min(680px, 95vw)', zIndex:2 }}>
            {renderPickerHeader('Stamp', selectedStamp ? `Sealing with ${STAMPS.find(s => s.id === selectedStamp)?.label ?? 'your stamp'}` : 'No stamp selected', 'Choose one if you want, then hop right back to the letter.', selectedStamp ? 'Use this stamp' : 'Continue without stamp')}
            <div style={{ textAlign:'center', marginBottom:'24px' }}>
