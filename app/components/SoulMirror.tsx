...
export default function SoulMirror({ isReturning = false, errorMessage = '', resumeState = null, onComplete }: SoulMirrorProps) {
  ...
  const [customFirstLetter, setCustomFirstLetter] = useState('')
  ...
  function buildAvatarDescription() {
    const editedDescription = freeformText.trim()
    if (editedDescription) return editedDescription
    return userAnswers.map((answer) => answer.trim()).filter(Boolean).join('\n')
  }

  function handleEnter() {
    const answersRecord: Record<number, string> = {}
    const finalAvatarDescription = buildAvatarDescription()
    ...
    if (avatarMode === 'freeform') {
      answersRecord[0] = finalAvatarDescription
  ...
  function previewCustomFirstLetter() {
    const trimmed = customFirstLetter.trim()
    if (!trimmed) {
      setLetterError('Write a little something before you continue.')
      return
    }
    setLetterError('')
    setGeneratedLetter(trimmed)
        ...
        {phase === 'chat' && avatarMode === 'guided' && (
          <motion.div key="chat" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.4 }}
            ...
            <div style={{ padding: '10px 16px 18px', borderTop: '1px solid rgba(255,255,255,0.07)', flexShrink: 0, marginTop: '8px' }}>
              {chatDone ? (
                <motion.button initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} onClick={() => {
                  if (!freeformText.trim()) {
                    setFreeformText(userAnswers.map((answer) => answer.trim()).filter(Boolean).join('\n'))
                  }
                  setPhase('freeform')
                }}
          ...
          <motion.div key="freeform" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.4 }}
            ...
            <SectionHeader
              ...
              sub={avatarMode === 'guided'
                ...
                : `Write as much or as little as you like. Style: ${selectedStyle?.label || customStyle.trim() || 'your own'}.`}
            />
            <textarea
              value={freeformText}
              ...
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '12px' }}>
                  <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    style={{ padding: '13px 24px', background: 'rgba(230,199,110,0.12)', border: '1px solid rgba(230,199,110,0.5)', color: letterSending ? 'rgba(230,199,110,0.4)' : '#e6c76e', fontFamily: "'Cinzel', serif", fontSize: '10px', letterSpacing: '0.25em', textTransform: 'uppercase', cursor: letterSending ? 'default' : 'pointer', borderRadius: '4px', transition: 'all 0.2s' }}>
