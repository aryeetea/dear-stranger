...
function buildCompanionRules(details: string): string[] {
  const lower = details.toLowerCase();
  if (!hasCompanionSignal(details)) return [];
...
function buildAccuracyGuard(details: string): string {
  ...
  const hasExplicitSkinDescription = /\bblack\b|\bwhite\b|\basian\b|\blatina\b|\blatino\b|\bbrown skin\b|\bdark skin\b|\bdark-skinned\b|\bdeep brown skin\b|\blight skin\b|\bfair skin\b|\bpale skin\b|\btan skin\b|\bolive skin\b|\bblue skin\b|\bice blue skin\b|\bgolden skin\b/.test(lower);
...
async function describeCurrentAvatarWithVision(openai: OpenAI, previousImageUrl: string, identityDescription?: string) {
  ...
  if (!summary) throw new Error("Could not read the current avatar well enough to edit it.");
  return summary;
}
...
export async function POST(req: Request) {
  try {
    ...
    const wantsFreshCharacter = explicitlyCreateNew || (!explicitlyEditCurrent && inferredFreshCharacter);
    ...
    if (shouldEditExisting && maskImageUrl) {
      // Convert URLs to Uploadable (Blob) as required by OpenAI SDK
      ...
      const [imageBlob, maskBlob] = await Promise.all([
        fetchImageAsBlob(previousImageUrl),
        fetchImageAsBlob(maskImageUrl),
      ]);
